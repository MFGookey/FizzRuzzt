...
pub async fn run() {
  warp::serve(routes::get_routes(FizzBuzzContext {counter : Arc::new(Mutex::new(1))})).run(([0,0,0,0], 9001))
  .await
}
...
pub mod routes {
  use warp::Filter;
  use crate::FizzBuzzContext;

  pub fn get_routes(current_state: FizzBuzzContext) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    let state_filter = warp::any().map(move|| current_state.clone());

    let ready = warp::get().and(warp::path("ready")).and_then(super::always_ok);
    let current = warp::get().and(warp::path("fizzbuzz")).and(state_filter).and_then(super::get_fizzbuzz);
